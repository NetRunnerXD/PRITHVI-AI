...
def extract(
    ...
    return {
        ...
        "visibility_m": current.get("visibility") if current.get("visibility") is not None else hraw("visibility"),
        ...
        else hraw("apparent_temperature"),
        "dew_point_c": current.get("dew_point_2m") if current.get("dew_point_2m") is not None else hraw("dew_point_2m"),
        "pressure_msl_hpa": current.get("pressure_msl") if current.get("pressure_msl") is not None else hraw("pressure_msl"),
        "surface_pressure_hpa": current.get("surface_pressure")
        if current.get("surface_pressure") is not None
        else hraw("surface_pressure"),
        "rain_now": current.get("rain") if current.get("rain") is not None else hraw("rain"),
        "showers_now": current.get("showers") if current.get("showers") is not None else hraw("showers"),
        "snowfall_now": current.get("snowfall") if current.get("snowfall") is not None else hraw("snowfall"),
        "wind_gusts_now": current.get("wind_gusts_10m") if current.get("wind_gusts_10m") is not None else hraw("wind_gusts_10m"),
        "sunrise": d0("sunrise"),
        "sunset": d0("sunset"),
