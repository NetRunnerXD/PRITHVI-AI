...
Also set `GEMINI_API_KEY` and `GROQ_API_KEY`. Chat tries **Ollama → Gemini → Groq → deterministic snapshot text**. On Render, loopback Ollama is skipped so the request does not wait on `127.0.0.1`. Set `LLM_FALLBACK=gemini,groq`. Snapshots rebuild every 10 minutes (`SNAPSHOT_TTL_S=600`).
