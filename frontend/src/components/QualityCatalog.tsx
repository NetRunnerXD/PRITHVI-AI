...
function Stat({ k, v }: { k: string; v: string }) {
  return (
    <div className="neo-in rounded-xl px-2 py-2">
      <p className="text-[10px] uppercase tracking-widest text-neo-muted">{k}</p>
      <p className="mt-0.5 break-all font-mono text-sm font-semibold">{v}</p>
    </div>
  );
}
...
function Sector({ title, note, children }: { title: string; note?: string; children: ReactNode }) {
  return (
    <section className="neo p-4">
      <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-neo-accent">{title}</p>
      {note ? <p className="mt-1 text-[11px] text-neo-muted">{note}</p> : null}
      <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">{children}</div>
    </section>
  );
}
...
export function QualityCatalog({ dash }: { dash: DashboardSnapshot }) {
  ...
  return (
    <div className="space-y-3">
      <p className="px-1 text-[11px] font-bold uppercase tracking-[0.16em] text-neo-muted">
        Quality variables — each field from the parameter list
      </p>

      <Sector
        title="Air quality"
        note="CPCB + Open-Meteo CAMS for gases. Pollen is India aerobiology climatology (CAMS pollen is Europe-only). Dash = not in the fetched sample."
      >
        <Stat
      ...
      <Sector title="Humidity & dew / apparent temperature">
      ...
      <Sector title="Pressure, cloud, visibility, ET">
